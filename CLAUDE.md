...
Optional Convex-backed **multi-sheet** storage, signed in through the **Neorgon Auth Kit**: `js/neorgon-auth.js`, `js/neorgon-auth-sites.js` and `css/neorgon-auth.css` are vendored from `packages/neorgon-ui/auth/` by `sync-auth.sh`, so never edit them here. One Neorgon account works on every Neorgon site. The kit owns the header slot (`.neo-auth`, in `.header-right` before `.header-home`), the sign-in dialog and the Convex token; `events.js` only listens with `NeoAuth.onChange` and gates `createNewSheet` / `saveCurrentSheet` with `NeoAuth.requireSignIn`. The Clerk JWT template must be named `convex`.

**Setup:** Run `npm install && npx convex dev` from the project root; set `CONVEX_URL` in `js/state.js`. The key in `<meta name="clerk-publishable-key">` is the fleet's production key, so sign-in cannot be exercised on localhost: the key refuses it, and the dialog says so (the kit README's Local development section has the `pk_test_` override). What the CSP must allow for sign-in is the kit README's CSP table, and `sync-auth.sh --check` fails on anything missing.

**Auth flow:** `initCharacterSheetAuth` (awaited from `app.js`, before the first render) registers one `NeoAuth.onChange` listener, then calls `NeoAuth.start({ convex })`. The listener runs once with the settled state, then only on a real change, never on a token refresh. It sets `state._user` to `{ label }` or `null`, clears `state._sheetId` / `state._sheetName` whenever the account changes (signing out included), shows or hides `#sheetsBar` (`render.js` repeats that from `state._user` on every render), and on sign-in fills `#sheetSelect` from `sheets:list`. `state._user` is session-only, never in `player-card` localStorage. `convex/migration.ts` (legacy password linking) is still deployed and `sheets.ts` still honours existing links, but nothing in the UI has created a link since 2026-09-10.

**Sheets:** Rows keyed by `ownerSubject` (JWT `subject`). `convex/sheets.ts`: `list` (authenticated), `save`, `remove`. The sheets bar appears when signed in.
