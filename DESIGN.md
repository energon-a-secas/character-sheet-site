...
description: Neorgon dark tool shell with a rotating per-section neon glow, a horizontal interview carousel, and a canvas-rendered RPG card export. All base suite tokens, plus eight section accents; sign-in is the fleet's Auth Kit.
colors:
  bg: "#040714"
...
**This is the Neorgon suite shell with two deliberate deviations.** Read the root [DESIGN.md](../DESIGN.md) first; everything there holds unless noted below. The deviations are: a **rotating per-section glow** and a **horizontal interview carousel** instead of a scrolling page. Sign-in is not a deviation: it is the fleet's Auth Kit, the same dialog every Neorgon site with accounts uses. The deliverable is a **canvas-rendered RPG card** the user exports.
...
Sign-in is the Neorgon Auth Kit (`packages/neorgon-ui/auth/`). The header slot, the dialog and every style they use are kit-owned and themed from this site's `--accent`, so this file defines none of them. The sheets bar appears once someone is signed in.

## Do's and Don'ts

### Do

- **Do** keep base suite chrome (header gradient, footer, font, blue primary) untouched; only layer the glow and carousel on top.
...
- **Don't** build a sign-in modal, sheet or `.auth-*` styles here; sign-in belongs to the Auth Kit.
